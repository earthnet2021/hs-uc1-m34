...
st.set_page_config(layout="wide",page_title="Vegetation Forecasts Africa Demo",page_icon=":earth:")	
...
def load_model(checkpoint):
    ...
    print("there is a weir checkpoint dict commente here")
    # Task
    task_args = [
    ...
    print("starting task load from checkpoint")
...
def visualisation_targ(targs):
    ...
    rgb = torch.cat(
        [
            sentinel[0, -10:, 3, ...].unsqueeze(3) * 4,
            sentinel[0, -10:, 2, ...].unsqueeze(3) * 4,
            sentinel[0, -10:, 1, ...].unsqueeze(3) * 4,
        ],
        dim=3,
    ).cpu().detach().numpy()
    ...
    st.write("Vegetation index for target period (groundtruth)")
    cols = st.columns(grid.shape[0])
    for col_num in range(grid.shape[0]):
...
def visualisation_pred(preds, score):
    ...
    for col_num in range(grid.shape[0]):
        cols[col_num].image(grid[col_num, ...], use_column_width=True)
        cols[col_num].text(f"Day t+{col_num*10+10}")
    return
...
if __name__ == "__main__":
    ...
    track = "iid"
    print("just started and loaded paths to things")

    setting_dict = parse_setting(setting, track)

    st.title("Forecast vegetation greeness on Cycle I Earthnet2022 dataset (EDConvLSTM)")
    ...
    if result:    
        ...
        with st.empty():
            st.write("Calculating forecasts...")
            with torch.no_grad():
